...
The main point was to develop, test and validate the code generated by mcparse, but also features the modernized component initialization using 4x4 affine transformations, custom visualization hooks for mcdisplay/mcplot, and exemplifies how to write custom trace algorithms, logging and visualization, and more.


# Liberate verified physics code

Years and decades has gone into writing, testing and verifying much of the mcstas components, which make them quite valuable. Unfortunately these gems were not readily available to use in different contexts.

Developers with C knowledge can take the project and write their own, custom ray-tracing procedures for McStas. Every ported component and instrument is available as a C++ source file, and porting more components is straight-forward.

The mcparse/mctrace project is imited as any early version, but still - most hard problems and road-blocks have been solved: Demonstrated pathways to dynamically configure and combine as you please, apply algorithms, frameworks, methods, runtime data analysis', custom visuals and data formats, all with full C++ compliance.

Adding such advanced capabilities to a complex system becomes possible when the code is readily available, modular, quick and easy to run and debug, and does so without any required frameworks or build systems.

The physics is the same, yet now with many more possible capabilities.

So, what's the catch? The production/release version is a ways off. Realizing it would require a degree of funding and community support. It was a lot of fun though, learned a whole lot, and hereby consider the McStas DSL wheel re-invented!
